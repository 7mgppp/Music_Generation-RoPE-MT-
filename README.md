...
| **Embedding Dimension ($d_{\text{model}}$)** | `512` | Head dimension $d_k = 64$ |
| **Feed-Forward Dimension ($d_{\text{ff}}$)** | `2048` | Expansion factor of 4 with GELU activation |
| **Max Context Length ($L_{\text{max}}$)** | `1024` | 1024 tokens (~550 notes under BPE) |
| **Positional Encoding** | **RoPE** | Rotary Position Embeddings (base $\theta = 10000$) |
...
- **Impact**:
  ...
  - **Context doubling**: Extended model context to $L_{\text{max}} = 1024$, allowing the model to attend across **~550 notes** (30–60+ seconds of dense classical music).
...
| :--- | :--- | :--- | :--- |
| **[`remi_primed_t07.mid`](samples/remi_primed_t07.mid)** | **Primed Continuation** | $T=0.70, \text{top\_k}=30, \text{top\_p}=0.90$ | Conditioned on an initial classical piano motif; develops melodic themes with natural harmonic cadences and polyphonic accompaniment. |
| **[`remi_t09.mid`](samples/remi_t09.mid)** | **Cold-Start (Unprimed)** | $T=0.90, \text{top\_k}=40, \text{top\_p}=0.90$ | Generated entirely from `<BOS>` token with expressive velocity variation and dynamic sustain pedaling. |
